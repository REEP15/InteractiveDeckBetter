...
export function SalesDeck() {
  ...
  /** Only accept intro `canplay` after user taps Start — avoids production/CDN firing readiness while preintro should run. */
  ...
  useEffect(() => {
    if (!isIntroReady || !showIntro) {
      return;
    }
    introRef.current?.play().catch(() => {});
    preIntroRef.current?.pause();
  }, [isIntroReady, showIntro]);
  ...
  const startIntroExperience = () => {
    ...
    if (video) {
      ...
      const handler = () => {
        if (introArmedRef.current) {
          setTimeout(() => {
            setIsIntroReady(true);
          }, 100);
        }
        video.oncanplay = null; // cleanup to prevent multiple triggers
      };

      video.oncanplay = handler;
    }
  };
